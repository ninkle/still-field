...
Use **Feet / full body** and leave **Detect jumps · full body** checked in Camera alignment. **Detection sensitivity → More sensitive** is the new default, including for existing installations. It accepts fainter person detections and smaller jumps. Choose **Standard** if you see false triggers. Keep the camera fixed with the person's head and feet fully visible. Stand briefly until the status says **Ready for a jump**, then jump in place. The status shows **Airborne** and holds **Landing detected** for two seconds after a recognized landing, so you can distinguish recognition from the visible effect. If tracking is too slow, the status suggests reducing Render detail.

A rise produces a small takeoff disturbance. Landing produces a much wider, stronger compression plus a separate outer ring that travels through the same wave field as the footsteps. This has its own bounded force, rather than sharing the footstep strength limit; the quiet central ripple keeps its existing motion. Walking footsteps and standing breaths are suppressed during the jump. Repeated landings remain distinct and their strength is capped.

The detector uses whole-body bounding-box motion, not a pose or foot-contact model. It requires multiple fresh raised samples followed by a descent and return. It tolerates a brief missed detection (up to a 0.45-second sample gap), smaller full-body boxes, raised arms during flight, and bent knees at landing. Cropped or overlapping people, long gaps, and incomplete jumps remain ineligible. Grounded arm raises, crouches, single-frame spikes and stale raised boxes do not qualify in the synthetic tests. Real detection can still miss jumps or misinterpret motion; validate it with the installation camera. Jump detection is disabled in **Body center / cropped view**. Uncheck it to return to the lighter four-update/second tracking rate.
